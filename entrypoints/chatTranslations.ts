import { translate, observeNode, chatContainer, chatInput, commsContent, chatContent } from '../components/utility';
...
export default defineUnlistedScript(() => {
    ...
    let lang = 'en';
    addChatTrans();
    let chatTranslate = document.getElementById("opt_chat_translate") as HTMLInputElement;
    ...
    function addChatTrans() {
        ...
        translateButton.onclick = async () => {
            ...
            const result = await translate(chatInput.value, 'auto', langIn.value == "" ? 'en' : langIn.value) as [string, string];
    ...
    async function translateChatMessage(p: Element) {
        ...
        if (!t) return;
        try {
            const [trans, origin] = await translate(t, 'auto', lang) as [string, string];
