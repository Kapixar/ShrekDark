import { observeNode } from '../components/utility';
...
export default defineUnlistedScript(() => {
    ...
    const isTestEnvironment = document.URL.includes('test');
    const buttonConfigs = [
    ...
    const handleImageError = function (this: HTMLImageElement) { this.src = defaultIcon; };
    ...
    async function convertInvite(mess: Element) {
        ...
        const playerContent = colonIndex !== -1 ? messageText.slice(colonIndex + 2).replace(regex, '') : messageText.replace(regex, '');
        ...
        if (isTestEnvironment !== urlIsTest) {
            const inviteCard = generateInviteCard(`Invitation to ${url.slice(8, -32)} ship.`, url);
            ...
            try {
                ...
                const shipName = meta[0]?.getAttribute('content')?.slice(8, -13) || 'Unknown';
                const inviteCard = generateInviteCard(`Invitation to ${shipName}`, url);
                ...
                const inviteCard = generateInviteCard(`Invitation is broken or goes to LABS.`, url);
                inviteCard.className = 'invite-container';
                mess.appendChild(inviteCard);
            }
        }
